...
func TestPageURLIncludesTheRequestedView(t *testing.T) {
	const addr = "http://127.0.0.1:8765"
	openSession := filepath.Join(t.TempDir(), "session.jsonl")
	writeServerSession(t, openSession,
		`{"type":"user","timestamp":"2026-07-09T00:00:00Z","sessionId":"open","cwd":"/tmp","message":{"role":"user","content":"hello"}}`,
	)

	openServer := New(Config{OpenSession: openSession})
	wantSession := addr + "/?session=" + url.QueryEscape(openServer.openSessionKey())
	if got := openServer.pageURL(addr); got != wantSession {
		t.Fatalf("open pageURL = %q, want %q", got, wantSession)
	}
	if got := New(Config{MapOnly: true}).pageURL(addr); got != addr+"/?map=1" {
		t.Fatalf("map pageURL = %q", got)
	}
	if got := New(Config{}).pageURL(addr); got != addr {
		t.Fatalf("serve pageURL = %q", got)
	}
}
