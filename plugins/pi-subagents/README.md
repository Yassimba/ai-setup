...
### External Claude Code and Codex workflows

External CLI backends are capability-gated: an agent's `backend` frontmatter selects a runtime but does not authorize it. Only these user-invoked workflows request grants:

- `/fable-advisor` activates a tool-free Fable advisor for the current persisted Pi session. Pi can consult it up to three times by default with `advisor.ask`; later consultations resume the same Claude session and send only deterministic context deltas.
- `/deep-research` runs independent Fable and Codex proposers against one immutable repository snapshot, then asks Fable to aggregate their frozen reports. It writes the result under `ai-docs/research` and marks one-proposer results as degraded.

Both workflows bind grants to the role and workflow content, backend, and canonical executable path. TUI runs ask before recording a grant; headless runs need an existing exact grant. They use Claude Code and Codex subscription authentication, never provider API keys configured by this extension. Direct external-agent execution fails closed.
...
### `herdrWorkspace` (fork addition)

```json
{ "herdrWorkspace": true }
```

Run async Pi children as FULL interactive sessions and capability-gated external
Claude Code/Codex workflows (including Fable advisor and deep research) in panes of a dedicated
herdr workspace (default label `subagents`, object form
`{ "herdrWorkspace": { "workspaceLabel": "my-agents" } }` overrides it). The
workspace and panes are never focused. Every managed feature keeps working —
status, usage/cost, wait, artifacts, results — because the runner tails the
child's session file and synthesizes the same message events json mode emits.
The workspace shows only agent panes (the empty seed terminal is closed) and
panes form a two-row grid: panes 1-2 side by side, 3-4 below them (2x2),
pane 5 opens a third column, pane 6 fills below it, and so on.
Finished panes auto-close by default — the workspace is a live view of what
your subagents are doing right now. Set `"keepPanes": true` in the object form
to instead keep completed panes open (renamed `✓`/`✗`), so a finished subagent
can be inspected or even continued by hand for Pi children; external CLI panes remain
inspectable but are not resumable from the terminal after their process exits. If the Herdr
server is unreachable, the run uses the normal headless path and records `local-fallback`
in its receipt and metadata. Session files are shared with
resume, and per-tool progress granularity is coarser than json mode (updates
arrive per message). External commands use a private `0600` envelope rather than
putting prompts in Herdr arguments; their helper tees output into the pane and
atomically returns the same JSONL result used by local execution. Cancellation
and timeouts close only the exact pane. Deep research displays both concurrent
proposer panes before its aggregator pane.
