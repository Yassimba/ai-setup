...
### Added
- Added Herdr-visible execution for capability-gated external workflows, with private prompt envelopes, live pane output, exact cancellation/timeout cleanup, shared grid placement, and advisor/deep-research topology.
- Added capability-gated Claude Code and Codex CLI backends, a resumable session-scoped Fable advisor, and a two-proposer Fable/Codex deep-research workflow with immutable snapshots, evidence-graded reports, deterministic receipts, and user-only activation skills.
...
### Fixed
- Recreate artifact directories at the final write boundary when another Pi lifecycle cleans them while a long-running child is active.
