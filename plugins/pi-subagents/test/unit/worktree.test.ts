...
import {
	...
	resolveWorktreeSetupHookCommand,
	type WorktreeSetup,
...
`);
	...
	describe("resolveWorktreeSetupHookCommand", () => {
		it("spawns the hook file directly on non-Windows platforms", () => {
			assert.deepEqual(
				resolveWorktreeSetupHookCommand("/repo/hooks/setup", "linux"),
				{ command: "/repo/hooks/setup", args: [] },
			);
		});

		it("dispatches Windows hooks to the interpreter matching the extension", () => {
			assert.deepEqual(
				resolveWorktreeSetupHookCommand("C:\\repo\\hook.cmd", "win32"),
				{ command: process.env.comspec ?? "cmd.exe", args: ["/d", "/c", "C:\\repo\\hook.cmd"] },
			);
			assert.deepEqual(
				resolveWorktreeSetupHookCommand("C:\\repo\\hook.BAT", "win32"),
				{ command: process.env.comspec ?? "cmd.exe", args: ["/d", "/c", "C:\\repo\\hook.BAT"] },
			);
			assert.deepEqual(
				resolveWorktreeSetupHookCommand("C:\\repo\\hook.ps1", "win32"),
				{ command: "powershell.exe", args: ["-NoProfile", "-ExecutionPolicy", "Bypass", "-File", "C:\\repo\\hook.ps1"] },
			);
			for (const script of ["hook.js", "hook.mjs", "hook.cjs"]) {
				assert.deepEqual(
					resolveWorktreeSetupHookCommand(`C:\\repo\\${script}`, "win32"),
					{ command: process.execPath, args: [`C:\\repo\\${script}`] },
				);
			}
			assert.deepEqual(
				resolveWorktreeSetupHookCommand("C:\\repo\\hook.exe", "win32"),
				{ command: "C:\\repo\\hook.exe", args: [] },
			);
		});

		it("rejects Windows hooks without a runnable extension and lists supported types", () => {
			assert.throws(
				() => resolveWorktreeSetupHookCommand("C:\\repo\\setup-hook", "win32"),
				/not runnable on Windows.*\.cmd, \.bat, \.ps1, \.js, \.mjs, \.cjs, \.exe/s,
			);
			assert.throws(() => resolveWorktreeSetupHookCommand("C:\\repo\\hook.sh", "win32"), /not runnable on Windows/);
		});
	});
});
