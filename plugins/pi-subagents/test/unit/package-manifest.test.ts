...
test("external advisor and research surfaces are packaged and human-invoked only", () => {
	const packageJson = JSON.parse(fs.readFileSync(path.join(projectRoot, "package.json"), "utf-8"));
	assert.deepEqual(packageJson.pi.skills, ["./skills"]);
	for (const skill of ["fable-advisor", "deep-research"]) {
		const source = fs.readFileSync(path.join(projectRoot, "skills", skill, "SKILL.md"), "utf-8");
		assert.match(source, /^---\n[\s\S]*?disable-model-invocation:\s*true[\s\S]*?\n---\n/);
	}
	for (const [role, backend] of [["deep-research-proposer-fable", "claude-code"], ["deep-research-proposer-codex", "codex-cli"], ["deep-research-aggregator", "claude-code"]] as const) {
		const source = fs.readFileSync(path.join(projectRoot, "agents", `${role}.md`), "utf-8");
		assert.match(source, new RegExp(`^---\\n[\\s\\S]*?backend:\\s*${backend}[\\s\\S]*?\\n---\\n`));
	}
});
