...
`, "utf-8");
	...
	it("creates and updates agents with external backends", () => {
		const ctx = { cwd: tempDir, modelRegistry: { getAvailable: () => [] } };
		const created = handleCreate({ config: { name: "external-reviewer", description: "External", scope: "project", backend: "codex-cli" } }, ctx);
		assert.equal(created.isError, false);
		const filePath = path.join(tempDir, ".pi", "agents", "external-reviewer.md");
		assert.match(fs.readFileSync(filePath, "utf-8"), /^backend: codex-cli$/m);

		const updated = handleUpdate({ agent: "external-reviewer", config: { backend: "claude-code" } }, ctx);
		assert.equal(updated.isError, false);
		assert.match(fs.readFileSync(filePath, "utf-8"), /^backend: claude-code$/m);

		const invalid = handleUpdate({ agent: "external-reviewer", config: { backend: "shell" } }, ctx);
		assert.equal(invalid.isError, true);
		assert.match(readText(invalid), /config\.backend must be/);
	});
