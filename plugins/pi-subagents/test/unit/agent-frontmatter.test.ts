...
describe("agent frontmatter backend", () => {
	it("defaults omitted backend to pi and round-trips explicit external backends", () => {
		const dir = fs.mkdtempSync(path.join(os.tmpdir(), "pi-subagents-agent-backend-"));
		tempDirs.push(dir);
...
`);
		...
		assert.equal(worker?.backend, "claude-code");
		assert.equal(worker?.extraFields?.backend, undefined);
		assert.match(serializeAgent(worker!), /^backend: claude-code$/m);

		writeAgent(filePath, `---\nname: worker\ndescription: Worker\n---\n\nDo work\n`);
		assert.equal(discoverAgents(dir, "project").agents.find((agent) => agent.name === "worker")?.backend, undefined);
	});

	it("rejects invalid backend definitions", () => {
		const dir = fs.mkdtempSync(path.join(os.tmpdir(), "pi-subagents-agent-invalid-backend-"));
		tempDirs.push(dir);
		writeAgent(path.join(dir, ".pi", "agents", "worker.md"), `---\nname: worker\ndescription: Worker\nbackend: shell\n---\n\nDo work\n`);
		assert.throws(() => discoverAgents(dir, "project"), /invalid backend frontmatter/);
	});
});
