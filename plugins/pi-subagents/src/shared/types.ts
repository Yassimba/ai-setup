...
import type { CapabilityRequest } from "../runs/shared/capability-grants.ts";
...
export interface SingleResult {
	...
	usage: Usage;
	model?: string;
	/** Whether an external backend ran locally, visibly in Herdr, or locally after Herdr was unavailable. */
	externalExecutionMode?: "local" | "herdr" | "local-fallback";
...
export interface RunSyncOptions {
	...
	runId: string;
	index?: number;
	/** Internal, pregranted workflow identity for an external backend invocation. */
	externalCapabilityRequest?: CapabilityRequest;
	sessionDir?: string;
	sessionFile?: string;
	share?: boolean;
	outputPath?: string;
...
export interface HerdrWorkspaceConfig {
	enabled?: boolean;
	/** Label of the (never focused) herdr workspace hosting native panes. Defaults to "subagents". */
	workspaceLabel?: string;
	/** Keep finished panes open (renamed ✓/✗) instead of auto-closing them. Defaults to false. */
	keepPanes?: boolean;
}

export interface ExtensionConfig {
	asyncByDefault?: boolean;
	/** Run async children as full pi sessions in panes of an unfocused herdr workspace. */
	herdrWorkspace?: boolean | HerdrWorkspaceConfig;
...
export const SUBAGENT_ACTIONS = ["list", "get", "models", "create", "update", "delete", "eject", "disable", "enable", "reset", "status", "interrupt", "resume", "steer", "stop", "append-step", "doctor", "watchdog.status", "watchdog.check", "watchdog.configure", "watchdog.recommend-model", "schedule", "schedule-list", "schedule-status", "schedule-cancel", "advisor.activate", "advisor.ask", "advisor.status", "advisor.reset", "advisor.disable", "deep-research.start"] as const;
