...
import { runPiNativeHerdr, toNativeArgs, type HerdrNativeSettings } from "./herdr-native.ts";
...
import { selectBackend } from "../shared/external-backends.ts";
import { runExternalExecution } from "../shared/external-execution.ts";
...
interface SubagentRunConfig {
	...
	/** Run children as full pi sessions in panes of an unfocused herdr workspace. */
	herdrWorkspace?: HerdrNativeSettings;
}
...
async function runSingleStep(
	...
	if (selectBackend(step.backend) !== "pi") {
		const external = await runExternalExecution({
			agent: {
				name: step.agent,
				backend: step.backend,
				description: step.label ?? step.agent,
				tools: step.tools,
				model: step.model,
				thinking: step.thinking,
				systemPrompt: step.systemPrompt ?? "",
				systemPromptMode: step.systemPromptMode ?? "replace",
			...
			runId: ctx.id,
			index: ctx.flatStepCount > 1 ? ctx.flatIndex : undefined,
			source: "async",
			herdrWorkspace: activeHerdrWorkspace,
		});
		try { fs.writeFileSync(ctx.outputFile, external.finalOutput ?? "", "utf-8"); } catch {}
		return {
			agent: step.agent,
			output: external.finalOutput ?? "",
			exitCode: external.exitCode,
			error: external.error,
			model: external.model,
			attemptedModels: external.model ? [external.model] : undefined,
			modelAttempts: external.model ? [{ model: external.model, success: external.exitCode === 0 && !external.error, exitCode: external.exitCode, error: external.error, usage: external.usage }] : undefined,
			artifactPaths: external.artifactPaths,
			transcriptPath: external.transcriptPath,
			transcriptError: external.transcriptError,
			timedOut: external.timedOut,
			stopped: external.stopped,
			sessionFile: external.sessionFile,
	...
	for (let index = 0; index < candidates.length; index++) {
		...
		const childWatchdog = watchdogConfig.ok
			...
			: undefined;
		if (activeHerdrWorkspace && !step.sessionFile) {
			// Native runs tail the session file, so a path must be pinned up front.
			// Pre-assigned session files (single async runs) are reused so status
			// and resume targets stay consistent; the tail skips prior content.
			step.sessionFile = path.join(
				path.dirname(ctx.outputFile),
				`native-session-${ctx.id}-${ctx.flatIndex}.jsonl`,
			);
		}
		...
		const nativeRun = activeHerdrWorkspace && step.sessionFile
			? await runPiNativeHerdr({
				args: toNativeArgs(args, step.sessionFile),
				cwd: step.cwd ?? ctx.cwd,
				sessionFile: step.sessionFile,
				agentName: step.agent,
				runId: ctx.id,
				stepIndex: ctx.flatIndex,
				settings: activeHerdrWorkspace,
				env: { ...env, ...getSubagentDepthEnv(step.maxSubagentDepth) },
				piPackageRoot: ctx.piPackageRoot,
				piArgv1: ctx.piArgv1,
				transcriptWriter,
				onChildEvent: (event) => ctx.onChildEvent?.(event as ChildEvent),
				appendChildEvent: (event) => {
					if (!shouldPersistChildEvent(event)) return;
					appendDiagnosticJsonl(eventsPath, JSON.stringify({
						...event,
						subagentSource: "child",
						subagentRunId: ctx.id,
						subagentStepIndex: ctx.flatIndex,
						subagentAgent: step.agent,
						observedAt: Date.now(),
					}), typeof event.type === "string" ? event.type : undefined);
				},
			...
			: undefined;
		const run = nativeRun && !nativeRun.fallback ? nativeRun : await runPiStreaming(
			args,
			step.cwd ?? ctx.cwd,
			ctx.outputFile,
			env,
	...
	if (artifactPaths && ctx.artifactConfig?.enabled !== false) {
		// Another Pi lifecycle may clean the shared artifact directory while a long-running
		// child is still active. Recreate it at the write boundary rather than relying on
		// the directory created before the child started.
		fs.mkdirSync(path.dirname(artifactPaths.outputPath), { recursive: true });
...
let activeHerdrWorkspace: HerdrNativeSettings | undefined;

async function runSubagent(config: SubagentRunConfig): Promise<void> {
	activeHerdrWorkspace = config.herdrWorkspace;
