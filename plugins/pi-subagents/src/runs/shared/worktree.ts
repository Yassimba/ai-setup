...
function linkNodeModulesIfPresent(toplevel: string, worktreePath: string): boolean {
	...
	try {
		// "junction" makes directory links work on Windows without Developer
		// Mode or admin privileges; the type argument is ignored elsewhere.
		fs.symlinkSync(nodeModulesPath, nodeModulesLinkPath, "junction");
		return true;
	} catch {
...
/**
 * Windows cannot spawn a hook file directly unless it is a real executable,
 * so dispatch by extension to the matching interpreter. Non-Windows platforms
 * spawn the hook file as-is (shebang + execute bit).
 */
export function resolveWorktreeSetupHookCommand(
	hookPath: string,
	platform: NodeJS.Platform = process.platform,
): { command: string; args: string[] } {
	if (platform !== "win32") return { command: hookPath, args: [] };
	const extension = path.extname(hookPath).toLowerCase();
	switch (extension) {
		case ".exe":
			return { command: hookPath, args: [] };
		case ".cmd":
		case ".bat":
			return { command: process.env.comspec ?? "cmd.exe", args: ["/d", "/c", hookPath] };
		case ".ps1":
			return { command: "powershell.exe", args: ["-NoProfile", "-ExecutionPolicy", "Bypass", "-File", hookPath] };
		case ".js":
		case ".mjs":
		case ".cjs":
			return { command: process.execPath, args: [hookPath] };
		default:
			throw new Error(`worktree setup hook '${hookPath}' is not runnable on Windows; supported hook types: .cmd, .bat, .ps1, .js, .mjs, .cjs, .exe`);
	}
}
...
function resolveWorktreeSetupHook(
	...
	const { command, args } = resolveWorktreeSetupHookCommand(resolvedPath);
	return {
