...
import { createNativeSupervisorChannel, registerNativeSupervisorClient } from "../../intercom/native-supervisor-channel.ts";
...
export default function registerSubagentPromptRuntime(pi: ExtensionAPI): void {
	if (process.env[SUBAGENT_FANOUT_CHILD_ENV] === "1") {
		...
		const channel = createNativeSupervisorChannel(pi, state);
		...
		pi.on("session_shutdown", () => channel.dispose());
	}
