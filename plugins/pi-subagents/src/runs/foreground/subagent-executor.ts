...
import { handleFableAdvisorAction } from "../shared/fable-advisor.ts";
import { resolveHerdrWorkspaceSetting } from "../shared/herdr-workspace.ts";
import { deepResearchCapabilityRequests, runDeepResearch } from "../shared/deep-research.ts";
...
const MUTATING_MANAGEMENT_ACTIONS = new Set(["create", "update", "delete", "eject", "disable", "enable", "reset", "watchdog.configure", "advisor.activate", "advisor.ask", "advisor.reset", "advisor.disable", "deep-research.start"]);
interface TaskParam {
	agent: string;
	task: string;
	cwd?: string;
...
export function createSubagentExecutor(deps: ExecutorDeps): {
	...
	const execute = async (
		...
		if (action) {
			if (action === "deep-research.start") {
				if (deps.allowMutatingManagementActions === false) {
					return { content: [{ type: "text", text: "Action 'deep-research.start' is not available from child-safe subagent fanout mode." }], isError: true, details: { mode: "management", results: [] } };
				}
				const question = (paramsWithResolvedCwd.message ?? paramsWithResolvedCwd.task ?? "").trim();
				let sessionFile: string | null = null;
				try { sessionFile = ctx.sessionManager.getSessionFile() ?? null; } catch {}
				const outcome = await runDeepResearch({
					question,
					cwd: requestCwd,
					artifactsDir: getArtifactsDir(sessionFile, requestCwd),
					herdrWorkspace: resolveHerdrWorkspaceSetting(deps.config.herdrWorkspace),
					signal,
					confirmGrants: ctx.hasUI ? async () => {
						const requests = deepResearchCapabilityRequests();
						return ctx.ui.confirm(
							"Activate deep research?",
							`Grant exact, content-bound capability to ${requests.length} bundled external roles for this workflow? Executables: ${[...new Set(requests.map((request) => request.executable))].join(", ")}`,
						);
					} : undefined,
				});
				if (!outcome.ok) {
					return { content: [{ type: "text", text: `Deep research failed during ${outcome.stage}: ${outcome.reason}` }], isError: true, details: { mode: "management", results: [], deepResearch: outcome } } as AgentToolResult<Details>;
				}
				const gaps = outcome.gaps.length > 0 ? `\nGaps: ${outcome.gaps.join("; ")}` : "";
				return {
					content: [{ type: "text", text: `Deep research ${outcome.status}. Report: ${outcome.reportPath}\nSources: ${outcome.sourceCount}${gaps}\n\n${outcome.receipt}` }],
					details: { mode: "management", results: [], deepResearch: outcome },
				} as AgentToolResult<Details>;
			}
			if (action.startsWith("advisor.")) {
				if (deps.allowMutatingManagementActions === false) {
				...
				return handleFableAdvisorAction(paramsWithResolvedCwd, signal, ctx, resolveHerdrWorkspaceSetting(deps.config.herdrWorkspace)) as Promise<AgentToolResult<Details>>;
			}
