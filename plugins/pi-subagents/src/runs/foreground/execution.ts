...
export async function runSync(
	...
	if (selectBackend(agent.backend) !== "pi") {
		return runExternalExecution({
			agent,
			task,
			cwd: options.cwd ?? runtimeCwd,
			...
			tools: agent.tools,
			sessionFile: options.sessionFile,
			sessionEnabled: Boolean(options.sessionFile || options.sessionDir || options.share),
			timeoutMs: options.deadlineAt === undefined ? options.timeoutMs : Math.max(0, options.deadlineAt - Date.now()),
			...
			index: options.index,
			source: "foreground",
			capabilityRequest: options.externalCapabilityRequest,
		});
	}
