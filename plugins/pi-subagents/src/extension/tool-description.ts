...
EXPLICIT EXTERNAL WORKFLOWS (human activation and exact capability grants required):
• { action: "advisor.activate" | "advisor.status" | "advisor.reset" | "advisor.disable" } - manage the session-scoped, tool-free Fable advisor
• { action: "advisor.ask", message: "focused execution question" } - consult the activated advisor (maximum three consultations by default)
• { action: "deep-research.start", task: "focused research question" } - run the foreground two-proposer/one-aggregator research workflow and save its report
...
MANAGE / CONTROL:
• Use action without execution fields: list, get, models, create, update, delete, eject, disable, enable, reset, doctor, watchdog.status, watchdog.check, watchdog.recommend-model, watchdog.configure, advisor.activate, advisor.ask, advisor.status, advisor.reset, advisor.disable, deep-research.start.
