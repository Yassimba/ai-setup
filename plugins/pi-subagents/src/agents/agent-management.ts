...
function preservedAgentFrontmatterFields(agent: AgentConfig, cfg: Record<string, unknown>): Set<string> {
	...
	if (hasKey(cfg, "backend")) changed("backend");
...
function applyAgentConfig(target: AgentConfig, cfg: Record<string, unknown>): string | undefined {
	if (hasKey(cfg, "backend")) {
		if (cfg.backend === false || cfg.backend === "") target.backend = undefined;
		else if (cfg.backend === "pi" || cfg.backend === "claude-code" || cfg.backend === "codex-cli") target.backend = cfg.backend;
		else return "config.backend must be 'pi', 'claude-code', 'codex-cli', or false when provided.";
	}
...
function formatAgentDetail(agent: AgentConfig): string {
	...
	if (agent.backend) lines.push(`Backend: ${agent.backend}`);
