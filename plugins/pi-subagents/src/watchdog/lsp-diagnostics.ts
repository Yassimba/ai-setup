...
import { toSpawnableCommand, windowsExecutableCandidates } from "../runs/shared/executable-resolver.ts";
import type {
	WatchdogLspConfig,
...
function pathExecutableNames(name: string): string[] {
	...
	// Never probe the bare name on Windows: npm puts an unspawnable POSIX sh
	// script at exactly that name, next to the .cmd shim.
	return windowsExecutableCandidates(name);
}
...
async function collectWithTypeScriptLanguageServer(input: {
	...
	const spawnable = toSpawnableCommand(input.command.command, input.command.args);
