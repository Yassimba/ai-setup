...
# The release tag is per-plugin in the skills monorepo (herdr-reviewr-v<version>) and matches the
# manifest version, so a checkout always pulls its own release.
