...
/// The checked-out branch name, `None` on a detached `HEAD`.
pub fn current_branch(repo: &Path) -> Option<String> {
    git_line(repo, &["symbolic-ref", "--quiet", "--short", "HEAD"])
}

/// The abbreviated `HEAD` commit, `None` on an unborn branch.
pub fn head_short(repo: &Path) -> Option<String> {
    git_line(repo, &["rev-parse", "--short", "HEAD"])
}

/// A canonical forge API and repository target. `owner` may be a multi-segment namespace on
/// GitLab (subgroups: `group/subgroup`); on GitHub it is always a single segment.
...
impl RepoTarget {
    /// The forge-side project path — `owner/name`, GitLab's `fullPath` shape.
    #[must_use]
...
/// Classify one remote URL against every host the classifier trusts: the well-known hosts,
/// the config overrides, and each host the user authenticated `gh`/`glab` against
/// (`specs/forge-host.md` "Host resolution"). SSH aliases (`<host>-work`) resolve to their
/// canonical host before the provider verdict.
fn classify_remote(url: &str, classifier: &crate::forge::detect::HostClassifier) -> OriginIdentity {
    ...
    // The canonical host: the URL's own host when it is trusted directly, else — over SSH
    // only — the base of a trusted-host alias (`github.com-work`, `selfhosted.example.com-alt`).
    let canonical = if classifier.classify(&host_lower) != Classification::Unknown {
        Some(host_lower.clone())
    } else if transport == RemoteTransport::Ssh {
        classifier
            .known_hosts()
            .find(|known| is_alias(&host_lower, known))
    ...
    let provider = match classifier.classify(&canonical) {
        Classification::Provider(provider) => provider,
        // Both CLIs claim the host and no override breaks the tie — refuse loudly rather
        // than guess a forge; the empty-state hint names the override keys.
        Classification::Ambiguous | Classification::Unknown => {
            return OriginIdentity::Unsupported(canonical);
        }
    };
    ...
    if segments.iter().any(|s| s.is_empty()) {
        return OriginIdentity::Malformed(canonical);
    }
    // GitHub paths are exactly owner/name; GitLab allows nested subgroups, so everything
    // before the last segment is the namespace.
    let (owner, name) = match (provider, segments.as_slice()) {
        (crate::forge::Provider::Github, [owner, name]) => ((*owner).to_owned(), *name),
        (crate::forge::Provider::Gitlab, [namespace @ .., name]) if !namespace.is_empty() => {
            (namespace.join("/"), *name)
        }
        _ => return OriginIdentity::Malformed(canonical),
    };
...
pub fn pr_local(
    ...
    config_bases: &[String],
    classifier: &crate::forge::detect::HostClassifier,
) -> Result<PrLocal, GitFail> {
    let origin = origin_identity(repo, classifier)?;
...
fn origin_identity(
    ...
    let out = run_git(repo, &args)?;
    if out.status.success() {
        return Ok(classify_remote(String::from_utf8_lossy(&out.stdout).trim(), classifier));
    }
...
/// The private ref pinning a Deep Review session's baseline snapshot, so `git gc` cannot
/// prune it while the session lives (its tree has no other referent).
fn deep_baseline_ref(key_hash: &str) -> String {
    format!("refs/reviewr/deep-base/{key_hash}")
}

/// Pin (or re-point) a deep session's baseline object under its private ref.
pub fn pin_deep_baseline(repo: &Path, key_hash: &str, sha: &str) -> Result<()> {
    git(repo, &["update-ref", &deep_baseline_ref(key_hash), sha])?;
    Ok(())
}

/// Drop a deep session's baseline ref (End Deep Review). Refs are shared across
/// worktrees, so this cleans the main repo even when the session's worktree is removed.
pub fn drop_deep_baseline(repo: &Path, key_hash: &str) {
    let _ = git(repo, &["update-ref", "-d", &deep_baseline_ref(key_hash)]);
}
...
mod tests {
    ...
    fn origin_identity_parses_github_and_enterprise_remote_forms() {
        ...
        use crate::forge::detect::HostClassifier;
        let plain = HostClassifier::from_files(None, None, None, None);
        let enterprise = HostClassifier::from_files(None, None, Some("github.company.com"), None);
        ...
        assert_eq!(
            classify_remote("https://github.com/owner/repo.git", &plain),
            repo("github.com", "owner", "repo")
        );
        assert_eq!(
            classify_remote("https://github.com/owner/repo", &plain),
            repo("github.com", "owner", "repo")
        );
        assert_eq!(
            classify_remote("https://github.com/owner/repo/", &plain),
        ...
        assert_eq!(
            classify_remote("git@github.com:owner/repo.git", &plain),
            repo("github.com", "owner", "repo")
        );
        assert_eq!(
            classify_remote("ssh://git@github.com/owner/repo.git", &plain),
            repo("github.com", "owner", "repo")
        );
        assert_eq!(
            classify_remote("ssh://git@github.com:22/owner/repo.git", &plain),
            repo("github.com", "owner", "repo")
        );
        assert_eq!(
            classify_remote("git://github.com/owner/repo", &plain),
            repo("github.com", "owner", "repo")
        );
        // Multi-account SSH host alias (`~/.ssh/config` `Host github.com-work`).
        assert_eq!(
            classify_remote("git@github.com-work:owner/repo.git", &plain),
            repo("github.com", "owner", "repo")
        );
        assert_eq!(
    ...
    #[test]
    fn origin_identity_parses_gitlab_remotes_with_subgroups() {
        use crate::forge::Provider;
        use crate::forge::detect::HostClassifier;
        let plain = HostClassifier::from_files(None, None, None, None);
        let selfhosted =
            HostClassifier::from_files(None, None, None, Some("gitlab.selfhosted.example.com"));
        let repo = |host: &str, owner: &str, name: &str| {
            OriginIdentity::Repository(RepoTarget {
        ...
        // gitlab.com is well-known: no config, no CLI file needed.
        assert_eq!(
            classify_remote("git@gitlab.com:owner/repo.git", &plain),
            repo("gitlab.com", "owner", "repo")
        );
        // Nested subgroups fold into the namespace; the last segment is the project.
        assert_eq!(
            classify_remote("https://gitlab.com/group/subgroup/project.git", &plain),
        ...
        assert_eq!(
            classify_remote("git@gitlab.selfhosted.example.com-alt:owner/repo.git", &selfhosted),
            repo("gitlab.selfhosted.example.com", "owner", "repo")
        );
        // A deep path on GitHub stays malformed — subgroups are a GitLab shape.
        assert_eq!(
            classify_remote("https://github.com/a/b/c", &plain),
    ...
    fn origin_identity_keeps_alias_and_failure_states_distinct() {
        ...
        let enterprise = HostClassifier::from_files(None, None, Some("github.company.com"), None);
        assert_eq!(
            classify_remote("https://github.com-attacker/owner/repo", &plain),
        ...
        // A GitLab host nobody trusts (no well-known, no override, no CLI claim).
        assert_eq!(
            classify_remote("git@gitlab.internal.example:owner/repo.git", &enterprise),
            OriginIdentity::Unsupported("gitlab.internal.example".to_string())
        );
        assert_eq!(
            classify_remote("https://github.com/owner", &plain),
            OriginIdentity::Malformed("github.com".to_string())
        );
        assert_eq!(
            classify_remote("https://github.com", &plain),
            OriginIdentity::Malformed("github.com".to_string())
        );
        assert_eq!(
            classify_remote("https://gitlab.com", &plain),
        ...
        assert_eq!(classify_remote("/tmp/repo", &plain), OriginIdentity::Hostless);
        assert_eq!(classify_remote("file:///tmp/repo", &plain), OriginIdentity::Hostless);
        assert_eq!(
            classify_remote("file://github.com/owner/repo", &plain),
            OriginIdentity::Unsupported("github.com".to_string())
        );
        assert_eq!(
            classify_remote("ftp://github.com/owner/repo", &plain),
            OriginIdentity::Unsupported("github.com".to_string())
        );
    }
