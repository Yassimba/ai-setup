...
/// installed (Wayland `wl-copy`, or X11 `xclip`/`xsel`); Windows ships `clip`. OSC 52 is roadmap.
...
impl ExportTarget for Clipboard {
    ...
    fn export(&self, text: &str) -> Result<()> {
        let advice = if cfg!(windows) {
            "no clipboard tool found (clip.exe ships with Windows — ensure System32 is on PATH) — \
             use \"Add all to chat\" instead"
        } else {
            "no clipboard tool found (install wl-clipboard, xclip, or xsel) — \
             use \"Add all to chat\" instead"
        };
        let (cmd, args) = select_tool(CLIPBOARD_TOOLS, crate::proc::on_path).context(advice)?;
        let payload = stdin_payload(cmd, text);
...
/// The bytes a clipboard tool reads from stdin. Windows' `clip` has no UTF-8 mode — raw
/// UTF-8 is decoded as the OEM codepage (mojibake) — but a UTF-16LE BOM switches it to
/// Unicode, so `clip` gets UTF-16LE with a BOM. Every other tool takes UTF-8 as-is.
fn stdin_payload(cmd: &str, text: &str) -> Vec<u8> {
    if cmd == "clip" {
...
#[cfg(test)]
mod tests {
    use super::{CLIPBOARD_TOOLS, format_all, format_comment, select_tool, stdin_payload};
    ...
    #[test]
    fn clip_receives_utf16le_with_bom_and_other_tools_raw_utf8() {
        assert_eq!(stdin_payload("pbcopy", "héllo 🙂"), "héllo 🙂".as_bytes());
        let payload = stdin_payload("clip", "héllo 🙂");
        assert_eq!(payload[..2], [0xFF, 0xFE], "the BOM tells clip the stream is UTF-16LE");
        let units: Vec<u16> =
            payload[2..].chunks(2).map(|pair| u16::from_le_bytes([pair[0], pair[1]])).collect();
