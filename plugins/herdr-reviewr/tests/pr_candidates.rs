...
fn pr_local(repo: &Path, base: Option<&str>) -> Result<PrLocal, GitFail> {
    let config = defaults();
    let classifier =
        herdr_reviewr::forge::detect::HostClassifier::from_files(None, None, None, None);
    pr_local_with_config(repo, base, config.base_branches(), &classifier)
}
...
fn push_head_other_name_yields_the_remote_branch_before_the_local_name() {
    ...
    assert_eq!(
        ...
        OriginIdentity::Repository(RepoTarget {
            provider: herdr_reviewr::forge::Provider::Github,
...
fn origin_identity_uses_instead_of_rewrite_and_ignores_pushurl() {
    ...
    let enterprise = herdr_reviewr::forge::detect::HostClassifier::from_files(
        None,
        None,
