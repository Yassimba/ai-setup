...
Versions up to 0.11.0 are upstream [persiyanov/herdr-reviewr](https://github.com/persiyanov/herdr-reviewr)
history; `yassimba.reviewr` restarts its own line at 0.1.0 on top of upstream v0.11.0.
...
- Multi-line review comments on both forges: a ranged selection in a remote Changes diff drafts
  ...
  GitLab Draft Notes carry `position.line_range` with computed line codes
  (`sha1(path)_<old>_<new>`), lifting the previous GitLab single-line restriction. GitHub
  findings also read back with their full range in the anchor (`path:start-end`); GitLab's
  GraphQL exposes no range on read, so an incoming GitLab range comment still anchors to its
  end line. In the viewer, forge threads and pending inline drafts now mark every row their
  range covers (previously only local comments did), and a pending draft's card names its
  anchor: `draft · src/a.rs:4-9 · pending sync`.
- Picker-selected PRs/MRs now populate the Changes tab with their remote file list and unified
  diffs, without checking out the branch or fetching refs. Forge threads and replies are plotted
  on their anchored lines; new inline comments and PR/MR replies stay local until explicit grouped
  sync (`s`). GitHub creates one grouped review for inline drafts; GitLab stages Draft Notes and
  bulk-publishes them. Definite failures remain retryable; ambiguous write outcomes stay visible
  but are blocked from blind retries.
- All files now carries subtle added/modified/deleted gutter decorations for the active scope.
- The PR/MR comment reader resolves anchors into the fetched remote patch and shows surrounding
  code context, with the forge-provided hunk as fallback.
- The PR/MR picker now has live fuzzy search across number, title, source branch, author, and
  lifecycle, with substring-first ranking and keyboard navigation over filtered results.
- Built-in project switcher (`ctrl+p` on any tab): fuzzy-pick a project from the configured
  `switcher_roots` (default: the repo's sibling directories, zoxide-frecency ordered when
  available) and re-point the running sidebar at it in place — no close-and-reopen, no
  separate switcher plugin, and no effect on any other pane. Unsent comments demand a
  second `enter` before they are dropped. New config key: `switcher_roots`.

### Fixed
- GitLab Draft Note writes now declare `Content-Type: application/json`; `glab --input` does not
  infer it and GitLab otherwise rejects the sync with HTTP 415. Definite HTTP 4xx rejections stay
  retryable and no longer suggest that a read refresh retries the write.
- File-tree rows now mark files and ancestor folders that have local or remote review feedback;
  collapsed All-files folders also show when they contain changed files.

## [0.1.0] - 2026-07-11

### Added
- GitLab merge request support in the PR tab via `glab` — gitlab.com and self-hosted, with
  nested subgroup paths, pipeline jobs as checks, and discussions as comments.
- Provider auto-detection from `origin`: well-known hosts, then `github_host`/`gitlab_host`
  overrides, then any host the authenticated `gh`/`glab` CLI claims (config-file read, no
  network). New `gitlab_host` config key.
- Windows support: `windows` platform builds (x64 + arm64), `herdr/install.ps1` bootstrap,
  and per-platform manifest commands (`-win` action ids on Windows).
- `herdr-reviewr sidebar <toggle|open|close|auto-open>` subcommand replaces `herdr/sidebar.sh`
  — one cross-platform code path, no bash/jq dependency for actions.
- A distinct "host unreachable" degraded state (VPN walls, IP allowlists) with its own remedy.
- A PR/MR picker (`p` on the PR tab), in two sections: open PRs/MRs first, then the newest
  merged & closed with their CI fate (`merged · ✓ passed` / `✗ failed` / `closed`) and
  comment counts (open/resolved threads on GitLab, totals on GitHub) — pin any of them to
  browse its checks and comments; `esc` or a branch switch unpins.
- The header's number chip uses the forge's own sigil: `#42` on GitHub, `!42` on GitLab.

### Changed
- Rebranded to `yassimba.reviewr`, released from the `Yassimba/ai-setup` monorepo with
  per-plugin tags (`herdr-reviewr-v<version>`).
- `two-face` now uses the pure-Rust fancy-regex engine (drops the oniguruma C dependency).
