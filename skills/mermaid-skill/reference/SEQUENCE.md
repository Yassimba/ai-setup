...
`autonumber` as the first body line numbers every arrow — use whenever prose refers to individual steps.

## Grouping Participants

```mermaid
sequenceDiagram
  box rgb(240, 240, 240) Backend
    participant S as Server
    participant D as DB
  end
  participant C as Client
...
`create` must be immediately followed by a message **to** the new participant; `destroy` by a message to or from the dying one.
