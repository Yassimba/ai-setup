...
`<<interface>>`, `<<abstract>>`, `<<enumeration>>`, `<<service>>` — first line inside the class body. Generics use `~T~` in the declaration; relationship lines use the bare name.

### Namespaces, Direction, Notes
...
classDiagram
  ...
  Invoice --> Payment
  note for Invoice "Immutable once sent"
```
